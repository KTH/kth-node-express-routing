jest.mock('express', () => {
  ...
  _routerMockup.get = jest.fn().mockReturnThis()
  _routerMockup.post = jest.fn().mockReturnThis()
  _routerMockup.put = jest.fn().mockReturnThis()
  _routerMockup.delete = jest.fn().mockReturnThis()
  return {
    Router: jest.fn().mockReturnValue(_routerMockup),
    _routerMockup,
  }
})
  ...
  const _runBasicTestsOfApiRouter = middleware => {
    ...
    describe(_text, () => {
      ...
      it(`${green('allows')} adding an unsecured endpoint with ${bold('register()')}`, () => {
        const router = ApiRouter(middleware)
        const result = router.register(apiDefGetNoSecurity, jest.fn())
      ...
      if (middleware) {
        it(`${green('allows')} adding a secure endpoint with ${bold('register()')}`, () => {
          const router = ApiRouter(middleware)
          const result = router.register(apiDefPostWithValidSecurity, jest.fn())
          expect(result).toBe(ExpressRouterMockup)
        })
      } else {
        it(`${THROWS} an error when adding a secured endpoint with ${bold('register()')}`, () => {
          const router = ApiRouter()
          expect(() => router.register(apiDefPostWithValidSecurity, jest.fn())).toThrow(
...
describe(`The function ${bold('register()')} delivered by "ApiRouter(middleware)"`, () => {
  ...
  it(`${RETURNS} the underlying Express.js Router for chaining`, () => {
